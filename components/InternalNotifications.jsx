...
export default function InternalNotifications({ userId }) {
    ...
    try {
      const response = await fetch(
        process.env.NEXT_PUBLIC_SITE_URL +
          `/api/send-internal-notification?limit=50`
      )
  ...
  const markAsRead = async (notificationId) => {
    try {
      await fetch(
        process.env.NEXT_PUBLIC_SITE_URL + '/api/send-internal-notification',
        {
          method: 'PATCH',
  ...
  return (
    <div className="relative">
      ...
      <button
        ...
        className="relative p-1.5 sm:p-2 bg-[#3716a8] border-2 sm:border-3 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] sm:shadow-[3px_3px_0px_0px_rgba(0,0,0,1)] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] transition-all duration-150 active:shadow-none active:translate-x-[2px] active:translate-y-[2px] rounded-md"
      >
        <Bell className="w-4 h-4 sm:w-5 sm:h-5 text-white" />

        {unreadCount > 0 && (
          <span className="absolute -top-1.5 -right-1.5 bg-red-500 text-white text-[10px] font-bold rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center border border-black">
      ...
      {isOpen && (
        <div className="fixed sm:absolute right-2 sm:right-0 left-2 sm:left-auto mt-2 sm:w-96 bg-white border-4 border-black shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] z-50 max-h-[80vh] overflow-y-auto">
          ...
          {/* Header */}
          <div className="flex items-center justify-between p-4 bg-[#3716a8] border-b-4 border-black text-white">
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="p-1 hover:bg-[#2a0f86] transition-colors"
              >
          ...
          <div className="max-h-96 overflow-y-auto">
            {loading ? (
              <div className="p-8 text-center">
                <div className="animate-spin w-8 h-8 border-4 border-black border-t-[#3716a8] rounded-full mx-auto"></div>
                    ...
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span
                            className={`px-2 py-1 text-xs font-bold border-2 border-black ${getTypeStyles(
                              notification.type
                            )}`}
                          >
